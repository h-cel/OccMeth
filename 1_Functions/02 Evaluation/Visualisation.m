...
methods_occurrence = {'none', 'ssr', 'tda'};
methods_intensity = {'qdm', 'mbcn'};
indices = [];
...
while strcmp(str_more,'Y') ==  1
    ...
    if sum(strcmp(x2, methods_intensity(:))) == 0
        disp('Please check your answer for typos')
        continue
    end
...
for i = 1:cnt+1
    ...
    if i<4
        for j=1:num_indices
            ...
            p(j) = scatter(data_RBMB(j), data_RBO(j), 40, icons{j}, 'LineWidth', 5);
        end
        xlim([0, 1.4])
        ylim([0, 1.4])
        plot([1,1], [0,2], 'r--')
        ...
        axis square
    elseif i == 4 %Option to enable a separate legend at a good location
...
save(strcat(save_loc, '_',figure_name, '_bad_performers.mat'), 'bad_performers')
